...
void main() {
  ...
  float mid = 0.46 + focus * 0.04 + wake * 0.02 + pulse * 0.015;
  float lit = 0.66 + focus * 0.05 + wake * 0.02;
  float hot = 0.82 + nexus * 0.03 + sync * 0.02;
  ...
  vColor *= mix(0.68, 0.88, aMist);
  vAlpha = mix(0.32 + 0.28 * activity, 0.4 + 0.14 * activity, aMist);
...
void main() {
  ...
  halo += exp(-d * d * 4.2) * vGlow * 0.05;
  float a = (core + halo) * vAlpha;
  if (a < 0.007) discard;
  vec3 c = vColor * a;
  float peak = max(c.r, max(c.g, c.b));
  float cap = mix(0.7, 0.32, vMist);
  if (peak > cap) {
    c *= cap / peak;
  }
...
void main() {
  ...
  float warmId = smoothstep(0.02, 0.14, aColor.r - aColor.b) * smoothstep(0.02, 0.12, aColor.g);
  float flowStrength = max(violetId, max(cyanId, warmId));
  vAlpha = aAlpha * (0.32 + flowStrength * 0.2 + focus * 0.05 + wake * 0.02 + nexus * 0.02);
  vec3 violetTint = vec3(0.86, 0.24, 1.0);
  vec3 cyanTint = vec3(0.1, 0.94, 0.98);
  vec3 warmTint = vec3(1.0, 0.72, 0.18);
  vec3 materialColor = aColor;
...
export default function NexusOrganism({ events, label }: NexusOrganismProps) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const view = portrait
        ? lookAt(-0.05, 0.12, dist, 0.03, 0.16, 0)
        : lookAt(-0.04, 0.13, dist, -0.08, 0.17, 0);
