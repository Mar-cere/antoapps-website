...
void main() {
  ...
  vMist = aMist;
  vGlow = conv;
  float flowPhase = sin(pos.x * 7.4 + pos.y * 5.3 + sin(pos.y * 4.1) * 0.9 + pos.z * 3.2);
  float crossPhase = sin(-pos.x * 5.2 + pos.y * 6.7 - pos.z * 4.0 + 1.8);
  float violetFlow = smoothstep(0.2, 0.92, flowPhase);
  float cyanFlow = smoothstep(0.2, 0.92, -flowPhase);
  float tealFlow = smoothstep(0.45, 0.95, crossPhase) * (1.0 - max(violetFlow, cyanFlow) * 0.35);
  vec3 lumaWeights = vec3(0.22, 0.55, 0.23);
  float baseLuma = dot(aColor, lumaWeights);
  vec3 blueTint = vec3(0.24, 0.48, 1.0);
  blueTint *= baseLuma / max(0.001, dot(blueTint, lumaWeights));
  vec3 violetTint = vec3(0.76, 0.18, 1.0);
  violetTint *= baseLuma / max(0.001, dot(violetTint, lumaWeights));
  vec3 cyanTint = vec3(0.06, 0.96, 1.0);
  cyanTint *= baseLuma / max(0.001, dot(cyanTint, lumaWeights));
  vec3 tealTint = vec3(0.08, 1.0, 0.8);
  tealTint *= baseLuma / max(0.001, dot(tealTint, lumaWeights));
  vec3 materialColor = mix(aColor, blueTint, 0.12);
  materialColor = mix(materialColor, violetTint, violetFlow * 0.62);
  materialColor = mix(materialColor, cyanTint, cyanFlow * 0.7);
  materialColor = mix(materialColor, tealTint, tealFlow * 0.5);
...
void main() {
  ...
  gl_Position = pos;
  vSide = aSide;
  float flowPhase = sin(world.x * 7.4 + world.y * 5.3 + sin(world.y * 4.1) * 0.9 + world.z * 3.2);
  float crossPhase = sin(-world.x * 5.2 + world.y * 6.7 - world.z * 4.0 + 1.8);
  float violetFlow = smoothstep(0.2, 0.92, flowPhase);
  float cyanFlow = smoothstep(0.2, 0.92, -flowPhase);
  float tealFlow = smoothstep(0.45, 0.95, crossPhase) * (1.0 - max(violetFlow, cyanFlow) * 0.35);
  float flowStrength = max(violetFlow, max(cyanFlow, tealFlow * 0.8));
  vAlpha = aAlpha * (0.24 + flowStrength * 0.52 + focus * 0.34 + wake * 0.1 + nexus * 0.12);
  vec3 lumaWeights = vec3(0.22, 0.55, 0.23);
  float baseLuma = dot(aColor, lumaWeights);
  vec3 blueTint = vec3(0.24, 0.48, 1.0);
  blueTint *= baseLuma / max(0.001, dot(blueTint, lumaWeights));
  ...
  vec3 tealTint = vec3(0.08, 1.0, 0.8);
  tealTint *= baseLuma / max(0.001, dot(tealTint, lumaWeights));
  vec3 materialColor = mix(aColor, blueTint, 0.1);
  materialColor = mix(materialColor, violetTint, violetFlow * 0.68);
  materialColor = mix(materialColor, cyanTint, cyanFlow * 0.74);
  materialColor = mix(materialColor, tealTint, tealFlow * 0.54);
...
void main() {
  ...
  vec3 tealTint = vec3(0.08, 1.0, 0.8);
  tealTint *= baseLuma / max(0.001, dot(tealTint, lumaWeights));
  vec3 materialColor = mix(aColor, violetTint, violetFlow * 0.62);
  materialColor = mix(materialColor, cyanTint, cyanFlow * 0.68);
  vColor = mix(materialColor, tealTint, tealFlow * 0.5);
  vAlpha = aAlpha * (0.74 + flowStrength * 0.72) * (0.82 + 0.18 * sin(uTime * 0.35 + aPosition.z * 3.0));
  vBary = aBary;
}
`;
...
void main() {
  ...
  float a = (veil * 0.9 + fold) * vAlpha * 2.15;
  if (a < 0.003) discard;
  gl_FragColor = vec4(vColor * a, a);
}
`;
