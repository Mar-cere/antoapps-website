...
void main() {
  ...
  float cyanId = smoothstep(0.04, 0.22, aColor.g - aColor.r) * smoothstep(0.1, 0.38, aColor.b);
  float violetId = smoothstep(0.05, 0.26, aColor.b - aColor.g) * smoothstep(0.1, 0.36, aColor.r);
  float warmId = smoothstep(0.04, 0.2, aColor.r - aColor.b) * smoothstep(0.02, 0.16, aColor.g);
  vec3 violetTint = vec3(0.84, 0.22, 1.0);
  violetTint *= baseLuma / max(0.001, dot(violetTint, lumaWeights));
  vec3 cyanTint = vec3(0.12, 0.9, 0.96);
  cyanTint *= baseLuma / max(0.001, dot(cyanTint, lumaWeights));
  vec3 warmTint = vec3(1.0, 0.7, 0.22);
  warmTint *= baseLuma / max(0.001, dot(warmTint, lumaWeights));
...
void main() {
  ...
  vAlpha = aAlpha * (0.2 + flowStrength * 0.28 + focus * 0.22 + wake * 0.08 + nexus * 0.08);
  vec3 violetTint = vec3(0.84, 0.22, 1.0);
  violetTint *= baseLuma / max(0.001, dot(violetTint, lumaWeights));
  vec3 cyanTint = vec3(0.12, 0.9, 0.96);
  cyanTint *= baseLuma / max(0.001, dot(cyanTint, lumaWeights));
...
export default function NexusOrganism({ events, label }: NexusOrganismProps) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const dist = width / height < 0.9 ? 1.12 : 1.08;
